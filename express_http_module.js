...
function ExpressHttpObject(root_object_val) {
    ...
    this.setupLink = function (req, res) {
        var link = this.linkMgrObject().search_and_create(req.headers.my_name, 0);
        if (!link) {
    ...
    this.keepAlive = function (req, res) {
        ...
        var link = this.linkMgrObject().search(req.headers.my_name, my_link_id);
        if (!link) {
            res.send(this.jsonStingifyData(req.headers.command, req.headers.ajax_id, null));
    ...
    this.getLinkData = function (req, res) {
        ...
        var link = this.linkMgrObject().search(req.headers.my_name, link_id);
        if (!link) {
            res.send(this.jsonStingifyData(req.headers.command, req.headers.ajax_id, null));
    ...
    this.getNameList = function (req, res) {
        ...
        var my_link = this.linkMgrObject().search(req.headers.my_name, my_link_id);
        if (!my_link) {
            res.send(this.jsonStingifyData(req.headers.command, req.headers.ajax_id, null));
        ...
        if (my_link.link_id === 0) {
            res.send(this.jsonStingifyData(req.headers.command, req.headers.ajax_id, null));
            this.abend("getNameList", "null my_link = 0");
            return;
        }
        link_entry.keep_alive(my_link);

        var name_array = this.linkMgrObject().get_name_list();
        ...
        res.send(json_str);
        this.debug(true, "getNameList", "(" + my_link.link_id + ",0) " + req.headers.my_name + "=>server " + name_array_str);
    };
    ...
    this.getSessionData = function (req, res) {
        ...
        link = this.linkMgrObject().search(req.headers.my_name, link_id);
        if (!link) {
            res.send(this.jsonStingifyData(req.headers.command, req.headers.ajax_id, null));
    ...
    this.putSessionData = function (req, res) {
        ...
        if (my_session.session_id === 0) {
            res.send(this.jsonStingifyData(req.headers.command, req.headers.ajax_id, null));
            this.abend("putSessionData", "null my_session = 0");
            return;
        }
        ...
        if (req.headers.my_name === req.headers.his_name) {
            ...
            if (his_session.session_id === 0) {
                res.send(this.jsonStingifyData(req.headers.command, req.headers.ajax_id, null));
                this.abend("putSessionData", "null his_session = 0");
                return;
            }
        }
